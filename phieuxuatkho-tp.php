<?php
    ...
    $nvl = json_encode($nvl);
    $pxks = include_once('database/showPhieuXuatKhotp.php');
    $count = include_once('database/countPhieuXuatKhotp.php');
                ...
                <div class="row-add">
                    <div class="column-add column-5">
                        <div class="dashboard_content_main">
                            ...
                            <h1>Lập phiếu xuất kho thành phẩm</h1>
                            <label for="thanhPham">Chọn thành phẩm cầu xuất kho:   </label>
                            <select id="thanhPham" onchange="loadLohangTP()">
                                <!-- Tùy chọn thành phẩm sẽ được đổ vào đây từ PHP -->
                                <?php include('database/showTPCode.php');?>
                            </select>
                            <br>
                            <br>
...
<script>
    // Hàm loadLohang được gọi khi chọn thành phẩm
    function loadLohangTP() {
        var mathanhPham = document.getElementById('thanhPham').value;
        ...
        xhr.open('GET', 'database/get-lohangTP.php?maThanhPham=' + mathanhPham, true);
        xhr.send();
    }
    ...
    $(document).on('click', '#updateUserBtn', function() {
        ...
        if (updatedmaLoHang && updatedsoLuongXuat) {
            var xhr = new XMLHttpRequest();
            var url = 'database/save-phieuxuatkhotp.php';  // Đặt đường dẫn tới file PHP xử lý phía server
